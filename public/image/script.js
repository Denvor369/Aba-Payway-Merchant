...
(function(){
  ...
  function getLang(){ return localStorage.getItem('siteLang') || localStorage.getItem('lang') || 'kh'; }
  function setLang(l){
  try{ localStorage.setItem('siteLang', l); localStorage.setItem('lang', l); 
  try{ document.dispatchEvent(new CustomEvent('site:langchange', { detail: { lang: l } })); }catch(_e){} 
}catch(e){}
  document.documentElement.lang = (l==='kh'?'kh':'en');
  document.body.classList.toggle('lang-kh', l==='kh'); localStorage.setItem('siteLang', l); document.documentElement.lang = l; document.body.classList.toggle('lang-kh', l === 'kh'); translatePage(l); renderProductControls(); }
  ...
  function translatePage(lang){
    ...
    $$('.product-card').forEach(card => {  return; });
  }
  ...
  || (card?.closest('main')?.querySelector('.pd-viewport img,#main,.pd-gallery .pd-viewport img')?.src)
  ...
  function changeQtyById(id, delta){
    ...
    if (idx === -1){
      if (delta > 0){
        ...
        const price = Number(card?.dataset?.price) || 0;
        bag.push({ id, title, price, qty: delta, img: (card?.dataset?.img)
        || (card?.closest('main')?.querySelector('.pd-viewport img,#main,.pd-gallery .pd-viewport img')?.src)
        || (card?.querySelector('img')?.src)
        || '' });
      } else return;
...
/*LANG_INIT*/(function(){try{var L=getLang(); if(!L){L='kh';} document.documentElement.lang=(L==='kh'?'kh':'en'); document.body.classList.toggle('lang-kh', L==='kh');}catch(e){}})();
...
(function(){
  try{
    var lang = getLang();
    if(lang!=='kh' && lang!=='en'){ lang = 'kh'; }
    // Sync selector if present
    var sel = document.getElementById('lang-switcher'); if(sel) sel.value = lang;
    setLang(lang);
  }catch(e){ try{ setLang('kh'); }catch(_e){} }
})();


// === Contact form i18n + lightweight submit ===
(function(){
  try{
    var TXT = {
      en: {
        title: "Customer Support",
        sub: "Ran into Problems? Contact us",
        name: "Name*",
        phone: "Phone Number*",
        email: "Email*",
        subject: "Subject*",
      ...
      kh: {
        title: "ជំនួយអតិថិជន",
        sub: "មានបញ្ហាឬ? ទាក់ទងមកកាន់យើងខ្ញុំ",
        name: "ឈ្មោះ*",
        phone: "លេខទូរស័ព្ទ*",
        email: "អ៊ីមែល*",
        subject: "ប្រធានបទ*",
        message: "សារ",
        send: "ផ្ញើ",
        thanks: "អរគុណ! យើងបានទទួលសាររបស់អ្នក។"
      }
    };

    function lang(){ try{ return (localStorage.getItem('siteLang')||localStorage.getItem('lang')||document.documentElement.lang||'kh'); }catch(e){ return 'kh'; } }

    function applyContact(langCode){
      var t = TXT[langCode] || TXT.en;
      var el = function(id){ return document.getElementById(id); };

      var title = document.querySelector('[data-i18n="contact.supportTitle"]');
      if (title) title.textContent = t.title;
      var sub = document.querySelector('[data-i18n="contact.supportSubtitle"]');
      if (sub) sub.textContent = t.sub;

      var ln = el('label-name'); if (ln) ln.textContent = t.name;
      var lp = el('label-phone'); if (lp) lp.textContent = t.phone;
      var le = el('label-email'); if (le) le.textContent = t.email;
      var ls = el('label-subject'); if (ls) ls.textContent = t.subject;
      var lm = el('label-message'); if (lm) lm.textContent = t.message;

      var f = {
        name: el('support-name'),
        phone: el('support-phone'),
        email: el('support-email'),
        subject: el('support-subject'),
        message: el('support-message'),
        submit: el('support-submit')
      };
      if (f.name) f.name.placeholder = t.name;
      if (f.phone) f.phone.placeholder = t.phone;
      if (f.email) f.email.placeholder = t.email;
      if (f.subject) f.subject.placeholder = t.subject;
      if (f.message) f.message.placeholder = t.message;
      if (f.submit) f.submit.textContent = t.send;
    }
    ...
    document.addEventListener('DOMContentLoaded', function(){
      ...
      if (form && !form._wired){
        ...
        form.addEventListener('submit', function(e){
          e.preventDefault();
          // simple validation
          var req = Array.from(form.querySelectorAll('[required]'));
          var invalid = req.filter(function(i){ return !i.value || !String(i.value).trim(); });
          if (invalid.length){
            invalid[0].focus();
            invalid.forEach(function(i){ i.classList.add('invalid'); setTimeout(function(){ i.classList.remove('invalid'); }, 1200); });
            return;
          }
          // toast
          (function toast(msg){
            try{
              ...
              t.className = 'add-toast';
              t.textContent = (TXT[lang()]||TXT.en).thanks;
              Object.assign(t.style,{position:'fixed',left:'50%',transform:'translateX(-50%)',bottom:'22px',background:'rgba(0,0,0,0.88)',color:'#fff',padding:'8px 12px',borderRadius:'8px',zIndex:99999,fontWeight:700});
              document.body.appendChild(t);
              setTimeout(function(){ try{ t.remove(); }catch(_e){} }, 1600);
            }catch(_) {}
          })();
          form.reset();
        }, false);
    ...
    // update on language change (script emits site:langchange)
    document.addEventListener('site:langchange', function(e){ try{ applyContact((e && e.detail && e.detail.lang) || lang()); }catch(_e){} }, false);
  }catch(e){ /* no-op */ }
})();


// === Simple "I'm not a robot" logic ===
(function(){
  function getLang(){ try{ return (localStorage.getItem('siteLang')||localStorage.getItem('lang')||document.documentElement.lang||'kh'); }catch(e){ return 'kh'; } }

  var CAPTCHA_TXT = {
    en: {
      robot: "I'm not a robot",
      verify: "Please verify you're not a robot."
    },
    kh: {
      robot: "ខ្ញុំ​មិន​មែន​ជា Robot ទេ",
      verify: "សូមបញ្ជាក់ថាអ្នកមិនមែនជា Robot។"
    }
  };

  function applyCaptchaI18n(code){
    var t = CAPTCHA_TXT[code] || CAPTCHA_TXT.en;
    var lbl = document.getElementById('captcha-text');
    if (lbl) lbl.textContent = t.robot;
  }

  function toggleSubmit(){
    var btn = document.getElementById('support-submit');
    var cb = document.getElementById('robot-check');
    if (!btn || !cb) return;
    btn.disabled = !cb.checked;
  }

  document.addEventListener('DOMContentLoaded', function(){
    applyCaptchaI18n(getLang());
    toggleSubmit();

    var cb = document.getElementById('robot-check');
    if (cb && !cb._wired){
      cb._wired = true;
      cb.addEventListener('change', toggleSubmit, false);
    }

    var form = document.getElementById('contact-form');
    if (form && !form._captchaGuard){
      form._captchaGuard = true;
      form.addEventListener('submit', function(e){
        var cb = document.getElementById('robot-check');
        var cap = document.getElementById('captcha-card');
        if (!cb || !cb.checked){
          e.preventDefault();
          if (cap){ cap.classList.add('invalid'); setTimeout(function(){ try{ cap.classList.remove('invalid'); }catch(_e){} }, 1200); }
  ...
  document.addEventListener('site:langchange', function(ev){
    var code = (ev && ev.detail && ev.detail.lang) || getLang();
    applyCaptchaI18n(code);
  }, false);
})();
